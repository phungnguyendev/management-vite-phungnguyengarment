...
const NAMESPACE = 'cut-group-embroideries'

export default {
  createItem: async (newItem: CutGroupEmbroidering, accessToken: string): Promise<ResponseDataType> => {
    return await client
      .post(
        `${NAMESPACE}`,
        {
  ...
  updateItemByPk: async (
    id: number,
    itemToUpdate: CutGroupEmbroidering,
    accessToken: string
  ...
  updateItemsBy: async (
    query: { field: string; id: number },
    itemsToUpdate: CutGroupEmbroidering[],
    accessToken: string
  ): Promise<ResponseDataType> => {
    return client
