...
import { SampleSewingSubmission } from '~/typing'
import { responseFormatter, throwErrorFormatter } from '~/utils/response-formatter'
const NAMESPACE = 'sample-sewing-submissions'

export default {
  createItem: async (newItem: SampleSewingSubmission, accessToken: string): Promise<ResponseDataType> => {
    return await client
      .post(
        `${NAMESPACE}`,
        {
  ...
  updateItemByPk: async (
    id: number,
    itemToUpdate: SampleSewingSubmission,
    accessToken: string
