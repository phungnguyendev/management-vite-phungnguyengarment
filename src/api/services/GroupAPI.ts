...
export default {
  createItem: async (newItem: Group, accessToken: string): Promise<ResponseDataType | undefined> => {
    return await client
      .post(
        `${NAMESPACE}`,
        {
  ...
  updateItems: async (itemsToUpdate: Group[], accessToken: string): Promise<ResponseDataType | undefined> => {
    return client
