...
export default {
  createItem: async (newItem: User, accessToken: string): Promise<ResponseDataType | undefined> => {
    return await client
      .post(
        `${NAMESPACE}`,
        {
  ...
  updateItems: async (itemsToUpdate: User[], accessToken: string): Promise<ResponseDataType | undefined> => {
    return client
